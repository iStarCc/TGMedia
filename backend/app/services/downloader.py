...
class SpeedTracker:
    """滑动窗口速度计算器，字节数停滞时自动归零"""

    def __init__(self, window_size: int = 10, stale_after: float = 2.0):
    ...
    def update(self, current_bytes: int) -> float:
        ...
        elif now - self._last_change_at >= self._stale_after:
            return 0.0
    ...
    async def _download(
        ...
        async with self.semaphore:
            ...
            try:
                ...
                last_progress = {"current": 0, "total": total_size or 1}
                last_db_flush = 0.0
                progress_tasks: set[asyncio.Task] = set()

                def track_progress_task(task: asyncio.Task) -> None:
                    progress_tasks.add(task)
                    task.add_done_callback(progress_tasks.discard)

                async def push_progress(
                    current: int, total: int, *, force: bool = False, persist: bool = True,
                ) -> None:
                    ...
                    if force:
                        await ws_manager.broadcast("task:progress", payload)
                    else:
                        await ws_manager.broadcast_progress(task_id, payload)

                    if not persist:
                        return
                    now = time.monotonic()
                    if force or now - last_db_flush >= 2 or current >= total:
                        await db.execute(
                ...
                def schedule_progress(current: int, total: int) -> None:
                    track_progress_task(asyncio.create_task(push_progress(current, total)))
                ...
                async def on_file_update(_c, update: td_types.UpdateFile):
                    ...
                    if local.is_downloading_completed:
                        ...
                        await push_progress(total, total, force=True)
                        download_done.set()
                        return

                    schedule_progress(current, total)
                ...
                if result.local:
                    current = getattr(result.local, "downloaded_size", 0) or 0
                    total = result.expected_size or total_size or 1
                    last_progress["current"] = current
                    last_progress["total"] = total
                    await push_progress(current, total, force=True)
                ...
                for task in list(progress_tasks):
                    if not task.done():
                        await task
