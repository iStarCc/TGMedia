  ...
  >
    ...
    <nav class="overflow-y-auto overscroll-contain py-1 -mx-1 px-1">
      <div class="flex flex-col gap-0.5">
        ...
        >
          ...
          <span
            v-else-if="item.path === '/settings' && hasUpdate"
    ...
    <div class="space-y-1 border-t border-surface-border pt-2">
      ...
      <a
        ...
        class="relative flex items-center gap-2 rounded-lg px-2 py-1.5 text-xs text-text-muted hover:text-text-secondary transition-colors"
      >
        <PhGithubLogo :size="14" />
        <span>v{{ current }}</span>
        <span
          v-if="hasUpdate"
          class="absolute right-1 top-1 h-1.5 w-1.5 rounded-full bg-danger"
        />
      </a>
    </div>
  </aside>
