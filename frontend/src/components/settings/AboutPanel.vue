...
const repoUrl = ref("https://github.com/iStarCc/TGMedia");
...
async function loadVersionInfo() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.repository) {
        repoUrl.value = `https://github.com/${data.repository}`;
      }
    }
  } finally {
    loading.value = false;
  ...
  <div class="space-y-6">
    ...
    >
      <div class="flex items-center gap-2 text-sm">
        ...
        <span>
          发现新版本 <strong class="font-mono">v{{ updateCheck.latest }}</strong>，当前 v{{ updateCheck.current }}
        </span>
      </div>
      <a
        :href="updateCheck.remote_url"
        target="_blank"
